...
[Changelog](../CHANGELOG.md) - notas de versão ligadas para cada versão publicada.
...
Se ainda usa uma versão antiga `v1.2.6.x`, atualize para `v1.3.14` ou mais recente antes de atualizar o Home Assistant Core para 2026.7 ou mais recente. As versões antigas `v1.2.6.x` exigiam `aiortc`, que não é compatível com o runtime Python 3.14 do Home Assistant. As versões atuais `v1.4.x` já não exigem `aiortc`.
...
A propriedade das gravações locais é registada por entrada da integração. Limpar uma entrada preserva as gravações mantidas por outra entrada. Os controlos de cache existentes permanecem inalterados.
...
O movimento da câmara está disponível como sensor binário Detetado/Livre e também como um evento Motion único para cada nova deteção. AI Detection continua a ser um evento único. Com `event.received`, `options.event_type` é obrigatório: `motion` para movimento ou um tipo de IA suportado, como `person`. O blueprint utiliza `xsense_camera_event`, filtrado pela entidade selecionada.
