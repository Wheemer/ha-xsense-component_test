...
[Changelog](../CHANGELOG.md) - 公開済み各バージョンのリンク付きリリースノート。
...
## 互換性と HACS 更新
古い `v1.2.6.x` ビルドをまだ使用している場合は、Home Assistant Core を 2026.7 以降へ更新する前に `v1.3.14` 以降へ更新してください。古い `v1.2.6.x` ビルドは `aiortc` を必要としていましたが、これは Home Assistant の Python 3.14 ランタイムと互換性がありません。現在の `v1.4.x` ビルドでは `aiortc` は不要です。
...
## SKP0A キーパッド コードの自動化
SKP0A キーパッドは、個々のキーの押下をすべて公開するわけではありません。有効な X-Sense アプリ作成コードが Home、Away、または Disarmed で送信された後、キーパッド イベントを発行します。統合により、送信されたコード イベントが `xsense_keypad_code` として公開されます。

付属の単一コード ブループリントを使用して、選択した 1 つのコードに対して Home Assistant アクションを実行します。オプションで、コードの送信に使用する Home、Away、または Disarmed モード ボタンを要求したり、オートメーションを 1 つのキーパッド シリアル番号に制限したりできます。

[![ブループリントのインポート](https://my.home-assistant.io/badges/blueprint_import.svg)](https://my.home-assistant.io/redirect/blueprint_import/?blueprint_url=https%3A%2F%2Fgithub.com%2FJarnsen%2Fha-xsense-component_test%2Fblob%2Fmain%2Fblueprints%2Fautomation%2Fxsense%2Fkeypad_code_action.yaml)

1 つのオートメーションで複数のキーパッド コードを異なる Home Assistant アクションにマップする場合は、ルーター ブループリントを使用します。

[![ルーターのブループリントをインポートする](https://my.home-assistant.io/badges/blueprint_import.svg)](https://my.home-assistant.io/redirect/blueprint_import/?blueprint_url=https%3A%2F%2Fgithub.com%2FJarnsen%2Fha-xsense-component_test%2Fblob%2Fmain%2Fblueprints%2Fautomation%2Fxsense%2Fkeypad_code_router.yaml)

キーパッド モード ボタンは送信ボタンとして機能します。選択したモードがすでにアクティブな場合、X-Sense は新しいイベントを発行できない可能性があるため、コードを送信するときに現在非アクティブなモード ボタンの 1 つを選択してください。
