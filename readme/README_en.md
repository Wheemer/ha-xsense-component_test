...
If you are still using an old `v1.2.6.x` build, update to `v1.3.14` or newer before upgrading Home Assistant Core to 2026.7 or newer. The old `v1.2.6.x` builds required `aiortc`, which is not compatible with Home Assistant's Python 3.14 runtime. Current `v1.4.x` builds no longer require `aiortc`.
...
When a Motion event includes X-Sense playback metadata, the integration prepares a private Home Assistant playback URL before sending a notification that opens the matching clip in X-Sense Recordings. Turn recording links off in the blueprint for a plain motion notification without video. Older imported X-Sense camera blueprints are updated automatically.

<!-- xsense-recording-storage-modes -->
Camera SD-card recordings appear in X-Sense Recordings. Playback only is the default storage mode: Home Assistant keeps signed X-Sense URLs private, rewrites the HLS playlist, and proxies segments only when the player requests them, without retaining complete clips. Keep local recordings stores complete clips under /media/xsense_recordings and enables configurable retention, a maximum size, manual deletion, and optional background sync. Local cleanup never deletes recordings from X-Sense SD cards or cloud storage.

<!-- xsense-cache-entry-ownership -->
Local recording ownership is tracked per integration entry. Clearing one entry preserves recordings retained by another entry. Existing cache controls remain unchanged.
...
Camera Motion is available both as a Detected/Clear binary sensor and as a one-time Motion event for each new detection. AI Detection is a one-time event. Motion events can include APK record playback metadata when X-Sense reports it; the integration prepares that metadata for proxied or retained playback before emitting the notification-ready event. The event data includes `recording_url`, `recording_media_url` when playback is ready, `snapshot_url`, and the full `playback` block. This is separate from AI Detection and does not require the AI service to be available. The blueprint exposes `xsense_event_type`, `xsense_recording_url`, `xsense_recording_media_url`, and `xsense_snapshot_url` for custom actions. For manual automations using `event.received`, `options.event_type` is required: use `motion` for Motion, or supported AI types such as `person`. The included blueprint uses the `xsense_camera_event` event bus, filtered by the selected entity, and works on the integration's minimum supported Home Assistant version.
