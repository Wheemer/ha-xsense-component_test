...
## Zgodność i aktualizacje HACS
Jeśli nadal używasz starej wersji `v1.2.6.x`, zaktualizuj ją do `v1.3.14` lub nowszej przed aktualizacją Home Assistant Core do wersji 2026.7 lub nowszej. Stare wersje `v1.2.6.x` wymagały `aiortc`, które nie jest zgodne ze środowiskiem Python 3.14 w Home Assistant. Aktualne wersje `v1.4.x` nie wymagają już `aiortc`.

Ta integracja jest instalowana jako niestandardowe repozytorium HACS. Jeśli Home Assistant nie pokazuje aktualizacji od razu, otwórz HACS, wybierz repozytorium X-Sense, użyj menu z trzema kropkami i uruchom **Update information**, a następnie zaktualizuj lub pobierz ponownie integrację i uruchom ponownie Home Assistant.
...
Gdy zdarzenie Motion zawiera metadane odtwarzania X-Sense, integracja przygotowuje prywatny adres URL odtwarzania Home Assistant przed wysłaniem powiadomienia, które otwiera pasujący klip w X-Sense Recordings. Wyłącz łącza do nagrywania w planie, aby uzyskać zwykłe powiadomienie o ruchu bez wideo. Starsze zaimportowane plany kamery X-Sense są aktualizowane automatycznie.
...
## SKP0A Automatyzacja kodu klawiatury
Klawiatura SKP0A nie publikuje każdego pojedynczego naciśnięcia klawisza. Publikuje zdarzenie klawiatury po przesłaniu prawidłowego kodu utworzonego przez aplikację X-Sense wraz z Home, Away lub Disarmed. Integracja ujawnia to zdarzenie przesłanego kodu jako `xsense_keypad_code`.

Użyj dołączonego schematu pojedynczego kodu, aby uruchomić akcje Home Assistant dla jednego wybranego kodu. Opcjonalnie możesz wymagać przycisku trybu Home, Away lub Disarmed używanego do przesłania kodu i opcjonalnie ograniczyć automatyzację do jednego numeru seryjnego klawiatury.

[![Importuj projekt](https://my.home-assistant.io/badges/blueprint_import.svg)](https://my.home-assistant.io/redirect/blueprint_import/?blueprint_url=https%3A%2F%2Fgithub.com%2FJarnsen%2Fha-xsense-component_test%2Fblob%2Fmain%2Fblueprints%2Fautomation%2Fxsense%2Fkeypad_code_action.yaml)

Użyj planu routera, jeśli chcesz, aby jedna automatyzacja mapowała kilka kodów klawiatury na różne akcje Home Assistant.

[![Importuj projekt routera](https://my.home-assistant.io/badges/blueprint_import.svg)](https://my.home-assistant.io/redirect/blueprint_import/?blueprint_url=https%3A%2F%2Fgithub.com%2FJarnsen%2Fha-xsense-component_test%2Fblob%2Fmain%2Fblueprints%2Fautomation%2Fxsense%2Fkeypad_code_router.yaml)

Przyciski trybu klawiatury działają jak przyciski wysyłania. Jeśli wybrany tryb jest już aktywny, X-Sense może nie opublikować nowego zdarzenia, dlatego podczas przesyłania kodu wybierz jeden z aktualnie nieaktywnych przycisków trybu.
