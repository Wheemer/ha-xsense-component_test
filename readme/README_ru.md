...
[Changelog](../CHANGELOG.md) - связанные примечания к выпуску для каждой опубликованной версии.
...
## Совместимость и обновления HACS
Если вы всё ещё используете старую сборку `v1.2.6.x`, обновитесь до `v1.3.14` или новее перед обновлением Home Assistant Core до 2026.7 или новее. Старые сборки `v1.2.6.x` требовали `aiortc`, который несовместим со средой Python 3.14 в Home Assistant. Текущие версии `v1.4.x` больше не требуют `aiortc`.
...
- **Камера (SSC0A, SSC0B)**: Показывает сущности камеры, миниатюры, нативный live view Home Assistant, диагностику и настройки на основе поведения Android-приложения, если это поддержано устройством и учетной записью.
...
Если событие Motion включает метаданные воспроизведения X-Sense, интеграция подготавливает частный URL-адрес воспроизведения Home Assistant перед отправкой уведомления, открывающего соответствующий клип в X-Sense Recordings. Отключите ссылки на запись в проекте, чтобы получать простое уведомление о движении без видео. Старые импортированные чертежи камеры X-Sense обновляются автоматически.

<!-- xsense-recording-storage-modes -->
Записи камеры на SD-карте появляются в X-Sense Recordings. Режимом хранения по умолчанию является только воспроизведение: Home Assistant сохраняет конфиденциальность подписанных URL-адресов X-Sense, перезаписывает список воспроизведения HLS и проксирует сегменты только тогда, когда проигрыватель их запрашивает, не сохраняя полные клипы. Хранить локальные записи хранит полные клипы под /media/xsense_recordings и обеспечивает настраиваемое время хранения, максимальный размер, удаление вручную и дополнительную фоновую синхронизацию. Локальная очистка никогда не удаляет записи с SD-карт X-Sense или облачного хранилища.

<!-- xsense-cache-entry-ownership -->
Принадлежность локальных записей отслеживается отдельно для каждой записи интеграции. Очистка одной записи интеграции сохраняет видеозаписи, удерживаемые другой. Существующие элементы управления кэшем остаются без изменений.
...
Движение камеры доступно и как бинарный датчик Обнаружено/Свободно, и как разовое событие Motion для каждого нового обнаружения. AI Detection — разовое событие. Для `event.received` параметр `options.event_type` обязателен: `motion` для движения или поддерживаемый тип ИИ, например `person`. Blueprint использует `xsense_camera_event` с фильтром по выбранной сущности.
