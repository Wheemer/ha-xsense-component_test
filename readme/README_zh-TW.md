...
[Changelog](../CHANGELOG.md) - 每個已發布版本的連結版本說明。
...
当 Motion 事件包含 X-Sense 播放元数据时，集成会在发送打开 X-Sense Recordings 中匹配剪辑的通知之前准备私有 Home Assistant 播放 URL。在藍圖中關閉錄製鏈接，以獲得不帶視頻的普通動作通知。較早匯入的 X-Sense 相機藍圖會自動更新。
