...
[Changelog](../CHANGELOG.md) - note de lansare legate pentru fiecare versiune publicată.
...
Când un eveniment Motion include metadate de redare X-Sense, integrarea pregătește o adresă URL privată de redare Home Assistant înainte de a trimite o notificare care deschide clipul potrivit în X-Sense Recordings. Dezactivați linkurile de înregistrare în plan pentru o notificare simplă de mișcare fără videoclip. Planurile mai vechi ale camerei X-Sense importate sunt actualizate automat.
...
## SKP0A Automatizări de cod de tastatură
Tastatura SKP0A nu publică fiecare apăsare de tastă individuală. Publică un eveniment de tastatură după ce un cod valid creat de aplicație X-Sense este trimis cu Home, Away sau Disarmed. Integrarea expune acel eveniment de cod trimis ca `xsense_keypad_code`.

Utilizați planul cu un singur cod inclus pentru a rula acțiuni Home Assistant pentru un cod selectat. Puteți solicita opțional butonul de mod Home, Away sau Disarmed utilizat pentru a trimite codul și, opțional, limitați automatizarea la un număr de serie al tastaturii.
...
Utilizați modelul routerului atunci când doriți ca o automatizare să mapeze mai multe coduri de tastatură la diferite acțiuni Home Assistant.

[![Importați planul routerului](https://my.home-assistant.io/badges/blueprint_import.svg)](https://my.home-assistant.io/redirect/blueprint_import/?blueprint_url=https%3A%2F%2Fgithub.com%2FJarnsen%2Fha-xsense-component_test%2Fblob%2Fmain%2Fblueprints%2Fautomation%2Fxsense%2Fkeypad_code_router.yaml)

Butoanele modului tastaturii acționează ca butoane de trimitere. Dacă modul selectat este deja activ, este posibil ca X-Sense să nu publice un eveniment nou, așa că alegeți unul dintre butoanele modului momentan inactiv atunci când trimiteți un cod.
