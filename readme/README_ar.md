...
إذا كنت ما زلت تستخدم إصدارًا قديمًا `v1.2.6.x`، فقم بالتحديث إلى `v1.3.14` أو أحدث قبل تحديث Home Assistant Core إلى 2026.7 أو أحدث. كانت الإصدارات القديمة تتطلب `aiortc`، وهو غير متوافق مع Python 3.14 في Home Assistant. الإصدارات الحالية `v1.4.x` لا تحتاج إلى `aiortc`.
...
عندما يتضمن حدث Motion بيانات تعريف تشغيل X-Sense، يقوم التكامل بإعداد عنوان URL خاص لتشغيل Home Assistant قبل إرسال إشعار يفتح المقطع المطابق في X-Sense Recordings. قم بإيقاف تشغيل روابط التسجيل في المخطط للحصول على إشعار بالحركة البسيطة بدون فيديو. يتم تحديث مخططات الكاميرا القديمة المستوردة X-Sense تلقائيًا.
