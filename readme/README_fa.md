...
[Changelog](../CHANGELOG.md) - یادداشت‌های انتشار لینک‌شده برای هر نسخه منتشرشده.
...
## سازگاری و به‌روزرسانی‌های HACS
اگر هنوز از نسخه قدیمی `v1.2.6.x` استفاده می‌کنید، قبل از ارتقای Home Assistant Core به 2026.7 یا جدیدتر، به `v1.3.14` یا جدیدتر بروزرسانی کنید. نسخه‌های قدیمی به `aiortc` نیاز داشتند که با Python 3.14 در Home Assistant سازگار نیست. نسخه‌های فعلی `v1.4.x` دیگر به `aiortc` نیاز ندارند.

این یکپارچه‌سازی به‌عنوان مخزن سفارشی HACS نصب می‌شود. اگر بروزرسانی فوراً نمایش داده نشد، HACS را باز کنید، مخزن X-Sense را انتخاب کنید، **Update information** را اجرا کنید، سپس یکپارچه‌سازی را بروزرسانی یا دوباره دانلود کنید و Home Assistant را راه‌اندازی مجدد کنید.
...
هنگامی که یک رویداد Motion شامل فراداده پخش X-Sense باشد، ادغام یک URL پخش خصوصی Home Assistant را قبل از ارسال اعلان آماده می کند که کلیپ منطبق را در X-Sense Recordings باز می کند. برای اعلان حرکت ساده بدون ویدیو، پیوندهای ضبط را در طرح اولیه خاموش کنید. طرح‌های اولیه دوربین X-Sense وارد شده به‌طور خودکار به‌روزرسانی می‌شوند.

<!-- xsense-recording-storage-modes -->
ضبط‌های کارت SD دوربین در X-Sense Recordings ظاهر می‌شوند. فقط پخش حالت ذخیره‌سازی پیش‌فرض است: Home Assistant نشانی‌های اینترنتی امضاشده X-Sense را خصوصی نگه می‌دارد، لیست پخش HLS را بازنویسی می‌کند، و بخش‌های پراکسی را فقط زمانی که پخش‌کننده درخواست می‌کند، بدون حفظ کلیپ‌های کامل، بازنویسی می‌کند. ضبط‌های محلی Keep کلیپ‌های کامل را تحت /media/xsense_recordings ذخیره می‌کند و حفظ قابل تنظیم، حداکثر اندازه، حذف دستی و همگام‌سازی پس‌زمینه اختیاری را فعال می‌کند. پاکسازی محلی هرگز موارد ضبط شده را از کارت‌های SD X-Sense یا فضای ذخیره‌سازی ابری حذف نمی‌کند.

<!-- xsense-cache-entry-ownership -->
مالکیت ضبط‌های محلی برای هر ورودی یکپارچه‌سازی جداگانه ثبت می‌شود. پاک کردن یک ورودی، ضبط‌های نگهداری‌شده توسط ورودی دیگر را حفظ می‌کند. کنترل‌های فعلی حافظه پنهان بدون تغییر باقی می‌مانند.
...
حرکت دوربین هم به‌صورت حسگر دودویی شناسایی‌شده/آزاد و هم به‌صورت رویداد یک‌باره Motion برای هر تشخیص جدید در دسترس است. AI Detection یک رویداد یک‌باره است. برای `event.received` تعیین `options.event_type` الزامی است: `motion` برای حرکت یا نوع پشتیبانی‌شدهٔ هوش مصنوعی مانند `person`. بلوپرینت از `xsense_camera_event` با فیلتر موجودیت انتخاب‌شده استفاده می‌کند.
