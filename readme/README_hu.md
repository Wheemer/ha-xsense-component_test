...
[Changelog](../CHANGELOG.md) - hivatkozott kiadási megjegyzések minden közzétett verzióhoz.
...
Ha még régi `v1.2.6.x` verziót használ, frissítsen `v1.3.14` vagy újabb verzióra, mielőtt a Home Assistant Core-t 2026.7 vagy újabb verzióra frissíti. A régi verziók `aiortc`-t igényeltek, amely nem kompatibilis a Home Assistant Python 3.14 futtatókörnyezetével. A jelenlegi `v1.4.x` verziók már nem igénylik az `aiortc`-t.
...
Ha egy Motion esemény X-Sense lejátszási metaadatokat tartalmaz, az integráció egy privát Home Assistant lejátszási URL-t készít, mielőtt értesítést küldene, amely megnyitja a megfelelő klipet a X-Sense Recordings fájlban. Kapcsolja ki a rögzítési linkeket a tervben, hogy sima mozgásértesítést kapjon videó nélkül. A régebbi importált X-Sense fényképezőgép tervrajzai automatikusan frissülnek.
